...
SELECT 
	...
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomer ON tkt.acct_id = dCustomer.AccountId AND dCustomer.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dCustomer.CustomerType = 'Primary' AND dCustomer.DimCustomerId > 0 AND dcustomer.IsDeleted = 0
		
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomerSalesRep ON tkt.acct_rep_id = dCustomerSalesRep.AccountId AND dCustomerSalesRep.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dCustomerSalesRep.CustomerType = 'Primary' AND dCustomerSalesRep.DimCustomerId > 0 AND dCustomerSalesRep.IsDeleted = 0
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomerTransSalesRep ON tkt.orig_acct_rep_id = dCustomerTransSalesRep.AccountId AND dCustomerTransSalesRep.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dCustomerTransSalesRep.CustomerType = 'Primary' AND dCustomerTransSalesRep.DimCustomerId > 0 AND dCustomerTransSalesRep.IsDeleted = 0
	...
	LEFT OUTER JOIN (
		SELECT ClassName, MAX(DimClassTMId) DimClassTMId
		FROM dbo.DimClassTM (NOLOCK)
		WHERE ETL_SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND DimClassTMId > 0
		GROUP BY ClassName
	) dctm ON dctm.ClassName = tkt.class_name
